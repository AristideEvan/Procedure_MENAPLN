...
<div class="container-fluid" >
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header py-0">{{ __('Ajouter un type promoteur') }}</div>
                    <div class="card-body">
                        <form class="needs-validation" novalidate method="POST" action="{{ route('typePromoteur.store') }}">
                            @csrf
                                ...
                                <div class="col-md-8 offset-md-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" required class="form-control" onkeyup="this.value = this.value.toUpperCase();" name="typeprom" id="typeprom"
                                            placeholder="Libellé du type Promoteur">
                                    </div>
                                    ...
                                    <fieldset>
                                        ...
                                        @foreach ($documents as $typeDocument => $data)
                                            <input type="checkbox" name="typeDocument[]" value="{{ $data['type']->id }}" class="parent" id="m{{ $data['type']->id }}">
                                            <label for="m{{ $data['type']->id }}">{{ $data['type']->libelle }}</label><br>

                                            @foreach ($data['documents'] as $document)
                                                <input type="checkbox" name="document[]" value="{{ $document->id }}" class="sm fils{{ $data['type']->id }}" id="sm{{ $document->id }}">
                            ...
                            <div class="form-group row mb-0">
                                <div class="col-md-6 offset-md-4">
                                    ...
                                    <a href="{{route('typePromoteur.index')}}/{{$rub}}/{{$srub}}"><input type="button" id="annuler" value={{__('Annuler')}} class="btn btn-primary btnAnnuler"/></a>
                                </div>
                            </div>
                        </form>
                    </div>
