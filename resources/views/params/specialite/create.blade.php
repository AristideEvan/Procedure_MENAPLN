        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header py-0">{{ __('Ajouter ') }}{{$libelle}}</div>
                    <div class="card-body">
                        <form class="needs-validation" novalidate method="POST" action="{{ route('specialite.store') }}">
                            @csrf
                                    ...
                                    <select id="libelleSecteur" type="text" class="form-control @error('libelleSecteur') is-invalid @enderror" name="libelleSecteur"
                                     required autofocus onchange="getDonnees('getFilieres',this.id,'libelleFiliere')">
                                        ...
                                        @foreach ($secteurs as $secteur)
                                            <option value="{{$secteur->id}}"> {{$secteur->libelleSecteur}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    value="{{ old('libelleSpecialite') }}" required autofocus>
                                    <div class="invalid-feedback">
                                        {{__('formulaire.Obligation')}}
                                    </div>
